...
class AudioWaveView : View {
    ...
    var progressColor: Int = Color.YELLOW
        set(value) {
            waveFilledPaint =
                    Paint().apply {
                        isAntiAlias = true
                        colorFilter = filterOf(value)
                    }
            postInvalidate()
        }

    var rangeColor: Int = Color.RED
        set(value) {
            rangePaint = smoothPaint(value.withAlpha(0xAA))
            postInvalidate()
        }
    ...
    private var rangeList : ArrayList<Range> = ArrayList()

    fun setRange(lower : Float,upper: Float, rangeColorSingle : Int){
        if(clearOnChange)
            rangeList.clear()

        rangeList.add(Range(lower,upper,rangeColorSingle))
        postInvalidate()
    }
    ...
    var clearOnChange: Boolean = false
        set(value) {
            field = value
        }

    private inner class Range(lower : Float,upper: Float, rangeColorSingle : Int) {

        //Setter
        var rangePaintSingle = Paint()
        val filter = LightingColorFilter(rangeColorSingle,1)

        var upperBound: Float = 0F
            set(value) {
                require(value in 0..100) { "Progress must be in 0..100" }

                field = Math.abs(value) / 100F
        ...
        init {
            rangePaintSingle.setColorFilter(filter)
            upperBound = upper
            lowerBound = lower
        }
    ...
    private var rangePaint = smoothPaint(rangeColor.withAlpha(0xAA))

    private var waveFilledPaint = Paint().apply {
                isAntiAlias = true
                colorFilter = filterOf(progressColor)
            }
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        if(rangeList.size != 0) {
            rangeList.forEach {
                cv.transform {
                    clipRect(w.toFloat() * it.lowerBound, 0F, w * it.upperBound, h.toFloat())
                    drawBitmap(waveBitmap, 0F, 0F, it.getPaint())
                }
        ...
        //Progress
        cv.transform {
            clipRect(0F, h.toFloat()/2, w * progressFactor, h.toFloat())
    ...
    private fun inflateAttrs(attrs: AttributeSet?) {
        ...
        with(resAttrs) {
            ...
            progressColor = getColor(R.styleable.AudioWaveView_progressColor, progressColor)
            rangeColor = getColor(R.styleable.AudioWaveView_rangeColor, rangeColor)
